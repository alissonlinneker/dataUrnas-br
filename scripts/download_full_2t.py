"""Download completo de todas as secoes eleitorais - 2o turno 2022."""
...
logger = logging.getLogger("download_full_2t")
...
async def download_all():
    ...
    async with TSEClient() as client:
        ...
        for i, uf in enumerate(ESTADOS_ORDENADOS):
            logger.info("=" * 60)
            logger.info("2T ESTADO %d/%d: %s", i + 1, len(ESTADOS_ORDENADOS), uf.upper())
            logger.info("=" * 60)
            ...
            logger.info(
                "2T ACUMULADO: %d secoes, %d downloads, %d erros | Tempo: %.0f min",
    ...
    elapsed = time.time() - start
    logger.info("=" * 60)
    logger.info("DOWNLOAD 2T COMPLETO!")
    ...
    logger.info("=" * 60)
    print(f"RESULTADO_FINAL_2T: {total_stats}")
